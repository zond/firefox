        ...
        loop {
            ...
            match self.inner.process(data, output_bufs.as_deref_mut())? {
                ProcessingResult::Complete { result: _ } => {
                    // For valid JXL images, basic_info and pixel_format are guaranteed
                    // to be the first outputs from the decoder. This check handles the
                    // transitional state before the image metadata header is fully parsed.
