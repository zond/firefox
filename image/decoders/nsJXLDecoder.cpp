...
LexerTransition<nsJXLDecoder::State> nsJXLDecoder::ReadJXLData(
  ...
  while (true) {
    JxlDecoderStatus decoder_status = jxl_decoder_process_data(
        mDecoder.get(), reinterpret_cast<const uint8_t**>(&aData),
        reinterpret_cast<uint32_t*>(&aLength));
...
nsresult nsJXLDecoder::ProcessFrame() {
  ...
  if (mCachedBasicInfo.is_animated) {
    ...
    if (mFrameIndex == 0) {
      ...
      PostLoopCount(
          mCachedBasicInfo.num_loops == 0 ? -1 : mCachedBasicInfo.num_loops);
    }
  ...
  if (mCachedBasicInfo.has_alpha) {
    ...
    if (!mCachedBasicInfo.alpha_premultiplied) {
