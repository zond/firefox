...
static void UnpackRowRGB24_To_(const uint8_t* aSrc, uint8_t* aDst,
  ...
  while (src >= aSrc) {
    ...
    uint8_t b = src[2];
#if MOZ_LITTLE_ENDIAN()
    constexpr bool actual_ARGB = std::is_same_v<aDest, ARGB>;
#else
    constexpr bool actual_ARGB = !std::is_same_v<aDest, ARGB>;
#endif
    if constexpr (actual_ARGB) {
...
SwizzleRowFn SwizzleRow(SurfaceFormat aSrcFormat, SurfaceFormat aDstFormat) {
  ...
  switch (FORMAT_KEY(aSrcFormat, aDstFormat)) {
    ...
    FORMAT_CASE_ROW(SurfaceFormat::CMYK, SurfaceFormat::X8R8G8B8,
                    UnpackRowRGB24_To_<ARGB>)
