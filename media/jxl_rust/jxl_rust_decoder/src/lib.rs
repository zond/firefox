    ...
    let alpha_for_compositing = match alpha {
        Some(alpha_buffer) => {
            ...
            for (idx, alpha) in buf.iter_mut().enumerate() {
                *alpha = f32::from_ne_bytes(alpha_buffer[idx * 4..idx * 4 + 4].try_into().unwrap())
                    as u8;
            }
            buf
        }
            ...
            if let Some(buf) = black {
                ...
                for y in 0..height {
                    for x in 0..width {
                        ...
                        tmp_color_buf[pixel_idx * 16..pixel_idx * 16 + 12]
                            .copy_from_slice(&original_colors[pixel_idx * 12..pixel_idx * 12 + 12]);
