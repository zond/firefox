...
/// Get image animation information from the decoder.
///
/// # Safety
/// - The decoder pointer must be valid and created by `jxl_rust_decoder_new`.
/// - The info pointer must be valid and point to writable memory.
#[no_mangle]
pub unsafe extern "C" fn jxl_rust_decoder_get_animation_info(
    decoder: *const JxlRustDecoder,
    info: *mut JxlRustAnimationInfo,
) -> JxlRustStatus {
    if decoder.is_null() || info.is_null() {
        return JxlRustStatus::Error;
    }
...
/// Get the current frame's information from the decoder.
///
/// # Safety
/// - The decoder pointer must be valid and created by `jxl_rust_decoder_new`.
/// - The info pointer must be valid and point to writable memory.
#[no_mangle]
pub unsafe extern "C" fn jxl_rust_decoder_get_frame_info(
    decoder: *const JxlRustDecoder,
    info: *mut JxlRustFrameInfo,
) -> JxlRustStatus {
    if decoder.is_null() || info.is_null() {
        return JxlRustStatus::Error;
    }
    ...
    let decoder = &*decoder;
    (*info).duration_ms = decoder.frame_duration;
